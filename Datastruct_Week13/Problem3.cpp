...
int SumofSubTree(tree* pos) {
	
	if (isExternalNode(pos->left) && isExternalNode(pos->right))
		return pos->elem;
	if (!isExternalNode(pos->left) && !isExternalNode(pos->right))
		return pos->elem + SumofSubTree(pos->left) + SumofSubTree(pos->right);
	if (!isExternalNode(pos->left))
		return pos->elem+SumofSubTree(pos->left);
	if (!isExternalNode(pos->right))
		return pos->elem+SumofSubTree(pos->right);
